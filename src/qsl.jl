...
struct QSL_OTOC{T <: Real}
    ...
    function QSL_OTOC(t::T, J::T, U::T, full_ham::BoseHubbard{T}, sys_ham::RBoseHubbard{T}, bath_ham::Vector{RBoseHubbard{T}}, eigenvecs::Matrix{T},rho::SparseMatrixCSC{ComplexF64, Int64}, is_thermal::Bool) where T <: Real
        state_bound = real(compute_state_bound(t, J, U, full_ham, sys_ham, bath_ham, eigenvecs, rho, is_thermal))
        spectral_bound = real(compute_spectral_bound(t, J, U, full_ham, sys_ham, bath_ham, eigenvecs, rho, is_thermal))
        new{T}(state_bound, spectral_bound)
    end

    
    function compute_state_bound(t::T, J::T, U::T, full_ham::BoseHubbard{T}, sys_ham::RBoseHubbard{T}, bath_ham::Vector{RBoseHubbard{T}}, eigenvecs::Matrix{T}, rho::SparseMatrixCSC{ComplexF64, Int64}, is_thermal::Bool) where T <: Real
        function integrand(time1::T, time2::T, J::T) where T <: Real
            ...
            rhob = is_thermal ? thermal_dm : partial_trace_system(rho_int_t, size(rho,1),full_ham.basis.N,full_ham.basis.M)
    ...
    function compute_spectral_bound(t::T, J::T, U::T, full_ham::BoseHubbard{T}, sys_ham::RBoseHubbard{T}, bath_ham::Vector{RBoseHubbard{T}}, eigenvecs::Matrix{T}, rho::SparseMatrixCSC{ComplexF64, Int64}, is_thermal::Bool) where T <: Real
        ...
        function inner_integral_spectral(time1::T) where T <: Real
            norm(quadgk(time2 -> liouv_superop(time1, time2, J), 0, time1)[1], Inf)
        end
